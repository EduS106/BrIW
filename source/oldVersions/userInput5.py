...
def draw_data(data, cols=1):
    if cols == 1:
        ...
        for dataset_num in range(0, len(data)):
            for counter in range(0, largest_dict(data)):
                user_id = counter + 1
                for dict_num in range(0, cols):
                    if len(data[dict_num]) >= user_id:
                        spacing = separator(data, data[dict_num][user_id])
                        print(f"[{user_id}]", data[dict_num][user_id], end=spacing)
                    else:
