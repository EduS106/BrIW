...
def start_app():
    ...
    while not user_exit:
        ...
        if user_selection > 6 or user_selection <= 0:
            ...
            editing_choice = edit_menu()
            if editing_choice == "ADD":
                drinks_dict = add_data("drink(s)", drinks_dict)
            elif editing_choice == "REMOVE":
                drinks_dict = remove_data("drink(s)", drinks_dict)
                pass
...
def draw_menu(menu_options, drawing=""):
    ...
    if cols == 1:
        ...
        for data_list in data:
            for item in data_list:
                ...
                if item_size > largest_item:
                    largest_item = item_size
    # print("Largest item is:", largest_item, "\nWith column:", cols, "\nWith data:", data,"\n")
    return largest_item
...
def largest_space(data, title, cols=1):
    ...
    if cols == 1:
        ...
        for list_num in range(0, cols):
            ...
            max_lengths.append(data_width)
            print("Adding max length:", max_lengths)
    print("Set of max lengths:", max_lengths)
    ...
    print("\nFinal letter space:", letter_space,"\n")
    return letter_space


def draw_line(cols=1, letter_space=0, largest_index=1):
    tabs_and_spaces = 7 * cols + 7 * (cols - 1)
    little_overhang = 2 * "="
    # " " * largest_index + " " + spacing, end = " " * largest_index + " "
...
def draw_table(title, data, cols=1):
    ...
    draw_data(data, cols, largest_index)
...
def remove_data(data_name, data):
    ...
    removed_data = input(f"\nPlease enter the name of the {data_name} you would like to remove, separated by commas: ")
    print()
    removed_data = removed_data.split(",")

    for item in removed_data:
        item_index = removed_data.index(item)
        item = item.strip()
        item = item.strip('"')
        item = item.strip("'")
        item = item.title()
        removed_data[item_index] = item

    if isinstance(data, list):
        for item in removed_data:
            data.remove(item)
    elif isinstance(data, dict):
        data = update_data(data_name, removed_data, data)
...
def update_data(data_name, input_data, dictionary, mode="add"):
    highest_id = 0
    if mode == "add":
...
def edit_menu():
    ...
    selected_option = 0
    while selected_option <= 0 or selected_option > len(possible_options):
        selected_option = draw_menu(possible_options)
        try:
