...
def add_data(data_name, data):
    ...
    if isinstance(data, list):
        ...
        data = update_data(data_name, added_data, data, "add")
...
def remove_data(data_name, data):
    ...
    removed_data = input(f"\nPlease enter the number(s) of the {data_name} you would like to remove, separated by commas: ")
    print()
    ...
    if isinstance(data, list):
        ...
        data = update_data(data_name, removed_data, data, "remove")
...
def save_to_file(data_name, data):
    ...
    elif data_name == "drink(s)":
        file = open("drinks.txt", "w")
    else:
        raise Exception("Error: only supports 'people' and 'drink(s)' for data_name string literal")
